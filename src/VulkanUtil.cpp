...
std::array<VkVertexInputAttributeDescription, 5> Vertex::getAttributeDescriptions() {
    ...
    attributeDescriptions[4].offset = offsetof(Vertex, face);
...
void VulkanUtil::initSurface(VkSurfaceKHR surface) {
...
void VulkanUtil::setupDebugMessenger() {
    if (!enableValidationLayers)
        return;
...
void VulkanUtil::createDescriptorSetLayout() {
...
void VulkanUtil::createColorResources() {
...
void VulkanUtil::createDepthResources() {
...
VkFormat VulkanUtil::findSupportedFormat(const std::vector<VkFormat> &candidates, VkImageTiling tiling, VkFormatFeatureFlags features) {
...
void VulkanUtil::createTextureImage() {
...
void VulkanUtil::generateMipmaps(VkImage image, VkFormat imageFormat, int32_t texWidth, int32_t texHeight, uint32_t mipLevels) {
...
VkSampleCountFlagBits VulkanUtil::getMaxUsableSampleCount() {
...
void VulkanUtil::createTextureImageView() {
    textureImageView = createImageView(textureImage, VK_FORMAT_R8G8B8A8_SRGB, VK_IMAGE_ASPECT_COLOR_BIT, mipLevels);
}

void VulkanUtil::createTextureSampler() {
...
VkImageView VulkanUtil::createImageView(VkImage image, VkFormat format, VkImageAspectFlags aspectFlags, uint32_t mipLevels) {
...
void VulkanUtil::createImage(uint32_t width, uint32_t height, uint32_t mipLevels, VkSampleCountFlagBits numSamples, VkFormat format, VkImageTiling tiling, VkImageUsageFlags usage, VkMemoryPropertyFlags properties, VkImage &image, VkDeviceMemory &imageMemory) {
...
void VulkanUtil::transitionImageLayout(VkImage image, VkFormat format, VkImageLayout oldLayout, VkImageLayout newLayout, uint32_t mipLevels) {
    VkCommandBuffer commandBuffer = beginSingleTimeCommands();
...
void VulkanUtil::createUniformBuffers() {
...
void VulkanUtil::createDescriptorPool() {
...
void VulkanUtil::createDescriptorSets() {
...
void VulkanUtil::createCommandBuffers() {
    ...
    for (size_t i = 0; i < commandBuffers.size(); i++) {
        ...
        clearValues[0].color = {0.1f, 0.4f, 1.0f, 1.0f};
...
void VulkanUtil::updateUniformBuffer(uint32_t currentImage) {
    void *data;
...
VkShaderModule VulkanUtil::createShaderModule(const std::vector<char> &code) {
...
SwapChainSupportDetails VulkanUtil::querySwapChainSupport(VkPhysicalDevice device) {
...
QueueFamilyIndices VulkanUtil::findQueueFamilies(VkPhysicalDevice device) {
...
bool VulkanUtil::checkValidationLayerSupport() {
    uint32_t layerCount;
...
VKAPI_ATTR VkBool32 VKAPI_CALL VulkanUtil::debugCallback(VkDebugUtilsMessageSeverityFlagBitsEXT messageSeverity, VkDebugUtilsMessageTypeFlagsEXT messageType, const VkDebugUtilsMessengerCallbackDataEXT *pCallbackData, void *pUserData) {
